...
export class StorageService {
  ...
  private readonly bucketName = process.env.GOOGLE_BUCKET_NAME ?? "ai-cv-evaluator";
  ...
  async saveProject(project: Express.Multer.File, cvData: CV) {
    let bucket = this.storage.bucket(this.bucketName);
    const path = this.getProjectFileFormat(cvData._id, project.originalname);
  ...
  async getProject(cvData: CV): Promise<Buffer> {
    const bucket = this.storage.bucket(this.bucketName);
    const path = this.getProjectFileFormat(cvData._id, cvData.project_filename);
    const file = bucket.file(path);
