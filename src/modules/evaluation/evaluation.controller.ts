...
export class EvaluationController {
  ...
  evaluate(@Body() evaluateDto: EvaluateDto, @GetUser() user: any) {
    return this.evaluationService.evaluate(evaluateDto, user.userId);
  }
  ...
  uploadCV(
    @UploadedFiles()
    files: { cv?: Express.Multer.File[]; project?: Express.Multer.File[] },
    @GetUser() user: any,
  ) {
    const cv = files.cv?.[0];
    ...
    return this.evaluationService.uploadCV({ cv, project }, user.userId);
  }
